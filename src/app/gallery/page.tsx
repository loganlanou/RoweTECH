...
export default function GalleryPage() {
  return (
    <>
      ...
      <section className="relative pt-32 pb-24 2xl:pt-40 2xl:pb-32 overflow-hidden">
        ...
        <div className="container-custom relative">
          <div className="max-w-3xl 2xl:max-w-4xl">
            <span className="text-primary-400 font-medium tracking-wider uppercase text-sm 2xl:text-base">Gallery</span>
      ...
      {/* Featured Section */}
      <section className="py-16 2xl:py-24 bg-secondary-800">
        <div className="container-custom">
          <div className="text-center mb-12 2xl:mb-16">
            <span className="text-primary-400 font-medium tracking-wider uppercase text-sm 2xl:text-base">Featured</span>
            <h2 className="section-heading mt-2">Machines in Action</h2>
          </div>

          <div className="grid md:grid-cols-2 gap-8 2xl:gap-12">
            {featuredImages.map((item, index) => (
              <div key={index} className="group relative rounded-2xl 2xl:rounded-3xl overflow-hidden border border-secondary-600 aspect-video">
                <Image
                  src={item.image}
                  alt={item.title}
                  fill
                ...
                <div className="absolute bottom-6 left-6 2xl:bottom-8 2xl:left-8">
                  <p className="text-white font-semibold text-lg 2xl:text-xl">{item.title}</p>
      ...
      {/* Category Filter */}
      <section className="py-8 2xl:py-10 bg-secondary-700 border-b border-secondary-600">
        <div className="container-custom">
          <div className="flex flex-wrap gap-2 2xl:gap-3">
            {categories.map((category) => (
              <button
                key={category}
                className={`px-4 py-2 2xl:px-5 2xl:py-2.5 rounded-full text-sm 2xl:text-base font-medium transition-colors duration-200 ${
                  category === 'All'
                    ? 'bg-primary-600 text-white'
                    : 'bg-secondary-800 text-secondary-300 hover:bg-secondary-600 border border-secondary-600'
                }`}
              >
                {category}
              </button>
      ...
      {/* Gallery Grid */}
      <section className="py-16 lg:py-24 2xl:py-32 bg-secondary-800">
        <div className="container-custom">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 2xl:gap-8">
            {galleryItems.map((item) => (
              <div
                key={item.id}
                className="group rounded-xl 2xl:rounded-2xl overflow-hidden border border-secondary-600 hover:border-primary-500/50 transition-all duration-300 hover:shadow-lg hover:shadow-primary-500/10"
              >
                <div className="aspect-square relative overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3 2xl:top-4 2xl:left-4">
                    <span className="bg-primary-600 text-white text-xs 2xl:text-sm font-medium px-2 py-1 2xl:px-3 2xl:py-1.5 rounded">
                      {item.category}
                    </span>
                  </div>
                </div>
                <div className="p-4 2xl:p-6 bg-secondary-900">
                  <h3 className="font-semibold text-white mb-1 2xl:text-lg group-hover:text-primary-400 transition-colors">{item.title}</h3>
                  <p className="text-secondary-400 text-sm 2xl:text-base">{item.description}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* CTA Section */}
      <section className="py-16 2xl:py-24 relative overflow-hidden">
        <div className="absolute inset-0">
          <Image
        ...
        <div className="container-custom relative text-center">
          ...
          <p className="text-secondary-300 text-lg 2xl:text-xl mb-8 max-w-2xl 2xl:max-w-3xl mx-auto">
